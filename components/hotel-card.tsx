...
export function HotelCard({
  ...
  function handleBook() {
    const detailUrl = `/hotels/${hotel.id}?checkIn=${checkIn}&checkOut=${checkOut}&adults=${adults}&rooms=${rooms}`;

    // Save booking intent — shown as "Resume booking" on dashboard if user returns
    savePendingBooking({
      type:       "hotel",
      label:      `${hotel.name}${hotel.stars ? ` · ${"★".repeat(hotel.stars)}` : ""} · ${checkIn} – ${checkOut}${hotel.price_per_night ? ` · ${formatINR(hotel.price_per_night)}/night` : ""}`,
      partnerUrl: detailUrl,
  ...
  const jsonLd = {
    ...
    "@type": "Hotel",
    "name": hotel.name,
    "description": hotel.excerpt || `Experience luxury at ${hotel.name} in ${hotel.city}.`,
    "image": hotel.media,
    "address": {
      "@type": "PostalAddress",
    ...
    "priceRange": hotel.price_per_night ? `₹${hotel.price_per_night} - ₹${Math.round(hotel.price_per_night * 1.5)}` : "Contact for pricing"
  };
  ...
  return (
    <>
      ...
      />
      <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone/10 hover:shadow-md transition-shadow overflow-hidden group">
          ...
          <div className="flex-1 p-5 flex flex-col">
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="min-w-0">
                <h3 className="font-display text-xl font-semibold text-ink dark:text-white leading-tight">{hotel.name}</h3>
                {hotel.chain && (
            ...
            {topAmenities.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {topAmenities.map((a: string) => {
                  ...
                  return (
                    <span key={a} className="flex items-center gap-1 text-[10px] text-stone bg-ivory dark:bg-stone-800 px-2 py-0.5 rounded-full">
